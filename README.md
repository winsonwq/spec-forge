...
**面向 Coding Agent 的 SPEC 产出工具。**

把模糊想法锻造成结构化、可测试、可执行的规格文档，产出物可直接指导 coding agent 完成开发、测试和验收。
...
## SPEC 产出物：Coding Agent 所需清单

SPEC 产出物覆盖了开发一个功能需要知道的所有方面：
...
| **范围** | 明确做什么 / 不做什么 |
| **技术方案** | 架构设计、算法逻辑、数据流 |
| **技术约束** | 语言/框架/依赖/项目结构（来自 project-analyzer） |
| **API 契约** | 接口定义、请求/响应格式、错误码 |
| **任务拆解** | 2-3 级任务，带依赖和工时 |
| **测试用例** | 功能/边界/异常/安全/性能测试 |
| **成功标准** | checklist，可直接验收 |
| **错误处理** | 场景 + 处理方式 |
| **验证通过** | logic-verifier 已跑通算法 |

有了这份 SPEC，Coding Agent 可以直接开始开发，无需额外追问。
