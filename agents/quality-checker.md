...
### Step 6: DRY 检查（Don&apos;t Repeat Yourself）

检查 SPEC 文档和测试用例中是否有不必要的重复：

**文档层面**（SPEC 章节之间）：
- 背景 / 目标 / 方案 / 测试验证 章节是否有重复表述
- 同一约束或规则是否在多处出现但表述不一致
- 任务拆解中的任务描述是否与其他章节重复

**测试用例层面**（test-designer 产出之间）：
- 测试用例之间是否有重叠的测试逻辑
- 相同前置条件是否重复出现在多个测试用例中
- 边界测试和功能测试之间是否有冗余

**任务拆解层面**（task-breaker 产出之间）：
- 任务之间是否有重复工作
- 依赖关系是否形成循环
